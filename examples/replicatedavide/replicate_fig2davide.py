 ...
 * num of trans = 8
 * data rate sweeped from 10^-2 to 10 pkt/s
...
for arrival_rate in arrival_rates:
    ...
    nDevices_values.append(np.ceil(totals_required[-1]/(simulationTime_values[0]/appPeriod_values[0])))
    appRates_values.append(np.ceil(simulationTime_values[0]*440/totals_required[-1]))
    print("For ", np.round(arrival_rate,3), " pkt/s. A total of ", np.round(totals_required[-1],3), " pkts in ",
           simulationTime_values[0], " seconds is required. Thus there must be ", np.round(nDevices_values[-1],3), " devices.")
    print("OR 440 devices must send at a period of ", appRates_values[-1], " s")
#print("Total packets received: ", totals_required, " in ", simulationTime_values[0], " seconds" )
#print("Total packets received: ", nDevices_values, " in ", simulationTime_values[0], " seconds" )
y = np.zeros(14)
    ...
    for confirmed_flag in param_combinations['confirmed']:
        ...
        for numNodes in appPeriod_values:
            ...
            if(index == 0 or index == 1): #First two are mac succ and phy succ, want to convert to loss percentage
                ...
                if(confirmed_flag):
                    print("      Calculated arrival rates was ", np.round(calc_arrival_rate[1],2), " goal ", np.round(arrival_rates[z],2))
                else:
                   print("       Calculated arrival rates was ", np.round(calc_arrival_rate[0],2), " goal ", np.round(arrival_rates[z],2)) 
            else:
